...
class PathFinder(object):
    ...
    def __init__(self, filePath, rootPath, checkExist=True, seek=False,
                 ...
                 autoResolveOSPaths=True, normalizePaths=True,
    ...
    def match_is_in_root(self):
        ...
        if not self.seek and \
                paths.is_current_dir(self.currentMatch, self.maxParentCount) and \
                paths.is_abs_path_in_current_dir(self.rootDir, self.currentMatch):
            return self.op.CONTINUE
        else:
            return self.op.PASS
    ...
    def match_differs_os(self):
        ...
        if pathOS == sysOS:
            ...
            print self.currentMatch, pathOS, sysOS
...
def change_path_to_current_os(path):
    ...
    winStuhome = r"H:\\"
    winNetwork = r"I:\\Savannah\\CollaborativeSpace\\"
    compileString = """
                    ^(?P<linuxTemp>{})?
                    (?P<winTemp>{})?
                    (?P<linuxStuhome>{})?
    ...
    for section in possibleMatchSections:
        ...
        if matchReturn is not None:
            ...
            return path.replace(matchReturn, replaceText)
