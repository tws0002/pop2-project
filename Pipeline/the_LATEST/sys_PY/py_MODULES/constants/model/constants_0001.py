...
__license__ = "MIT"
__version__ = '1.0.1'  # odd numbered minor value means it's a 'dev' build
...
REGISTERED_ADMINS = ('ckenne24', 'SUDO')
...
# MATCH_RE = r'''
#             ^((_+)?s0(?P<shot>\d{2})(?P<shotRevision>[A-Z])?_)?
#             (?P<user>[\x00-\x7F]{4,6}\d{2})_
#             (?P<version>0\d{2})(?P<versionRevision>[A-Z])?_
#             (?P<type>[A-Z]{3,5})
#             (_(?P<description>[a-zA-Z]+)?)?
#             ((_v|.)(?P<increment>0\d{3})(?P<incrementRevision>[A-Z])?).
#             ''' + _EXT
SHOT_MATCH_PATTERN = r"^((_+)?s0(?P<shot>\d{2})(?P<shotRevision>[A-Z]{1,2})?"\
...
# fileToSave = "s001_ckenne24_010_MODEL_someDescription.0001.tiff"
# fileToSave = "s001_ckenne24_010_MODEL_someDescription.#######.tiff"
# fileToSave = "s001_ckenne24_010_MODEL_someDescription.$F1203.tiff"
# fileToSave = "s001_ckenne24_010_MODEL_someDescription.%0234d.tiff"
# fileToSave = "ckenne24_010_MODEL_someDescription.0001.tiff"
# match = re.match(RE_FILENAME_MATCH, fileToSave)
# print match.groups()
# print match.group("increment")
...
if "APP_EXECUTABLE" in os.environ:
    ...
    if not os.path.isfile(os.environ['APP_EXECUTABLE']) \
            and not omnibus.is_executable(str(os.environ['APP_EXECUTABLE'])):
    ...
    if os.environ['APP_EXECUTABLE'] not in SUPPORTED_APPLICATIONS:
        ...
        raise globerror.ExitApplicationError
