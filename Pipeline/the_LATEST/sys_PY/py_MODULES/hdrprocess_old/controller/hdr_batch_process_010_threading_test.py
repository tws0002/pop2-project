...
def hdr_batch_process(inputs, sequenceInterval, *args, **kwargs):
    ...
    if kwargs.get('fileType', None) is None:
        fileType = '.cr2'
    else:
    ...
    if kwargs.get('recursive', None) is False:
        folderSearchLevel = 0
    else:
    ...
    if kwargs.get('autoRename', None) is None \
       and kwargs.get('inputFileNames', None) is None:
        sys.exit("No method to rename files specified")

    if kwargs.get('ci', None) is None and \
       kwargs.get('ec', None) is None or kwargs.get('defcurve'. None):
        sys.exit("No method for defining the camera curve specified")

    if kwargs.get('ci', None) is not None \
       and not os.path.isfile(kwargs.get('ci', None)):
