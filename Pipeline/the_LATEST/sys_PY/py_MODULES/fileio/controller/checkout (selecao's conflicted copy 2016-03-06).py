...
class FileIO(object):
    ...
    def __init__(self, filePath):
        ...
        self._farm_character_limit = 126
...
class FileCheckout(FileIO):
    ...
    def __init__(self, filePath):
        ...
        self.allInOrderArgs = ["filenamePrefix", "shotPrefix", "shot",
                               "shotRevision", "shotRedraft", "shotSuffix", "user",
                               "userSuffix", "version", "versionRevision",
                               "versionSuffix", "type", "descriptionPrefix",
                               "description", "incrementPrefix", "increment",
                               "incrementRevision", "incrementSuffix",
                               "extension"]
        self.requiredArgs = ['user', "userSuffix", 'version', "versionSuffix",
                             'type', "incrementPrefix", 'increment',
                             "incrementSuffix", 'extension']
        self.optionalArgs = ['filenamePrefix', 'shotPrefix', 'shot',
                             "shotRevision", "shotRedraft", "shotSuffix",
                             'versionRevision', "descriptionPrefix",
                             "description", 'incrementRevision', 'description']
        self._sceneParentFolders = ['Previs', 'Edit']

        regex = consts.RE_FILENAME_MATCH
        self._match = re.match(regex, self.fileName)

        self.sceneParentFolder = None

        # helper for the checkout list - I figured I would keep the list here
        # so that the GUI could respond and update to changes in the class
        #
        self.checkoutList = ["Is force save enabled", "Is user registered",
                             "Is working locally", "Is working remotely",
                             "has network access",
                             "Is correct file name",
                             "Is path under {} characte"\
                             "rs".format(self._sys_character_limit),
                             "Is project under {} charact"\
                             "ers".format(self._farm_character_limit),
                             "Is allowed to save",
                             "Is file locked"]
    # end __init__
    ...
    def is_complete_file_name(self):
        """
        Checks that the filename is satisfies the required terms of the class
        """
        if self.check_required_args["missing file args"] == []:
            return True
        else:
            return False
    # end is_complete_file_name

    def get_missing_required_args(self):
        pass
    # end get_missing_required_args
    ...
    def elevate_class_by_application(self, application):
        ...
        if application.lower() not in consts.SUPPORTED_APPLICATION_NAMES:
            ...
            app = FileCheckoutMaya(self.fullPath)
        elif application == "nuke":
            app = FileCheckoutNuke(self.fullPath)
        elif application == "houdini":
            app = FileCheckoutHoudini(self.fullPath)
        return app
    ...
    def check_required_args(self):
        ...
        rewrite
        """
        capturedRequiredArgs = []
        missingRequiredArgs = []
        for arg in self.requiredArgs:
            if self._match.group(arg) is not None:
                capturedRequiredArgs.append([arg, self._match.group(arg)])
            else:
                missingRequiredArgs.append(arg)
        return {"found required args": capturedRequiredArgs,
                "missing required args": missingRequiredArgs}
...
def test_incomplete_file_name():
    """
    Testing if there's a way to check if a filename is lacking and suggest
    specific improvements to the name
    """
    sampleFileName = r"s001_ckenne24_010_MODEL_asdfasdfdf.0001.ma"
    fileIO = FileCheckout(sampleFileName)
    print fileIO.allInOrderArgs
    finalString = ""
    requiredArgs = ""
    reMatch = re.match(consts.RE_FILENAME_MATCH, sampleFileName)
    for term in fileIO.allInOrderArgs:
        if reMatch.group(term) is not None:
            finalString += reMatch.group(term)
        if term in fileIO.requiredArgs:
            print term, reMatch.group(term)
            requiredArgs += reMatch.group(term)
    print sampleFileName
    print finalString
    print requiredArgs
    print fileIO.check_required_args()
# end test_incomplete_file_name


if __name__ == "__main__":
    # print(__doc__)
    test_incomplete_file_name()
