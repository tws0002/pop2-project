...
def os_path_split_asunder(path, debug=False, replaceSeparators=False):
    """
    ...
    Args:
        ...
        replaceSeparators (bool): If you expect a mixture of '/'s and '\'s,
                                  enable this to conform the path to a single
                                  slash type. This is particularly useful in
                                  Linux, when examining Windows OS paths
    Returns:
...
def search_parent_count(string, parentCount=3):
    """
    ...
    if match is None or match == []:
        ...
        return {'match': match[0],
                'count': len(match),
                'is_current_dir': is_current_dir(string, parentCount=parentCount)}
# end search_parent_count


def is_current_dir(string, parentCount=3):
    """
    ...
    Args:
        ...
        maxParentCount (int): The number of search levels up a relative path is
                              allowed to be before it's too high up. In Maya,
                              where the project file is considered the root, it
                              takes 3 ../s to get past the project folder, for
                              example
    ...
    if search_parent_count(string) is not None and \
       len(search_parent_count(string)['match']) == parentCount:
...
def is_abs_path_in_current_dir(rootDir, subFolder):
    """
    Checks if an absolute path exists within the current directory
    """
    rootDirSplit = os_path_split_asunder(rootDir)
    subFolderSplit = os_path_split_asunder(subFolder)
    if len(rootDirSplit) > len(subFolderSplit):
        # an absolute subfolder cannot be shorter than an absolute parent folder
        return False

    for rootDirF, subF in zip(rootDirSplit, subFolderSplit):
        if rootDirF != subF:
            break
    else:
        return True  # the function never broke
    return False
# is_abs_path_in_current_dir
