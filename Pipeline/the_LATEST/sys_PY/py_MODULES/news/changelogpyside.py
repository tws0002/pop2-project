...
import view.changeLogBasePySide as basePySide
import view.changeLogOKPySide as okPySide


CHANGELOG_KEY = "CHANGELOG_HIDE_ON_STARTUP"


class Window(QtGui.QMainWindow, basePySide.Ui_MainWindow):
    ...
    def add_love(self):
        ...
        """
        crewprefsDir = consts.CREW_PREFS_DIR_LOC
        crewprefsDir = [os.path.join(crewprefsDir, x) for x \
                        in os.listdir(crewprefsDir)]

        for crewmember in crewprefsDir:
            if os.path.basename(crewmember).startswith(consts.REGISTERED_ADMINS):
                with open(crewmember, "r") as f:
                        data = json.load(f)
                data['LOVE'] = data.get("LOVE", 0) + 1

                with open(crewmember, "w") as f:
    ...
    def set_config(self, ignoreAdmins=True):
        ...
        with open(consts.CREWMEMBER_PREFERENCES_LOC, "r") as f:
            data = json.load(f)

        hideUntilUpdate = self.hideUntilUpdate_cb.isChecked()

        if CHANGELOG_KEY in data:
            hideUntilUpdateConf = data[CHANGELOG_KEY]
        else:
        ...
        data[CHANGELOG_KEY] = hideUntilUpdate
        with open(consts.CREWMEMBER_PREFERENCES_LOC, "w") as f:
