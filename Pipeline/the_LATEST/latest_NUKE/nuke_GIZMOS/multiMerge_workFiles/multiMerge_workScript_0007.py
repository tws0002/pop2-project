...
thisNode = nuke.toNode('MultiMerge')
...
with thisNode:
    # RESET
    nAll = nuke.allNodes()
    masterMerge = nuke.nodes.Merge(name="MasterMerge")
    operation = int(masterMerge.knob('operation').getValue())
    masterMerge.knob('operation').setValue(operation)
    ...
    for i in xrange(mergeNum):  
        if i+1 != mergeNum:
            ...
            merge_h.knob('operation').setValue(operation)
