...
class MultiChannelSplitPanel(nukescripts.PythonPanel):
    ...
    def __init__( self ):
            ...
            self.autoCropPerChannel = nuke.Boolean_Knob("autoCropPerChannel",
                                                        "autocrop per channel",
                                                        0.0)
            self.autoCropPerChannel.setFlag(nuke.STARTLINE)
            ...
            self.addKnob(self.autoCropPerChannel)
            ...
            def multiChannelSplit():
                ...
                if sel != None:
                    ...
                    if self.autoCrop.getValue()==1.0:
                        curveNode = nuke.nodes.CurveTool(name="Autocrop_Master",
                                                         inputs=[sel],
                                                         operation="Auto Crop")
                        curveNode["channels"].setValue("rgba")
                        curveNode.knob("ROI").setValue([0,0,sel.width(),
                                                        sel.height()])
                        nuke.execute(curveNode, sel.knob("first").value(),
                                                         sel.knob("last").value())
                        masterCurveNode = curveNode  # saved to delete later
                    layersToProcess=[]
                    ...
                    if len(layersToProcess)>0:
                        ...
                        for channelLayer in layersToProcess:
                            ...
                            if self.autoCrop.getValue()==1.0 and \
                                        self.autoCropPerChannel.getValue() == 1.0:
                                # negate the previous nuke node and make new
                                nuke.delete(curveNode)
                                curveNode = nuke.nodes.CurveTool(
                                                         name="Autocrop_Master",
                                                         inputs=[sel],
                                                         operation="Auto Crop")
                                curveNode["channels"].setValue("rgba")
                                curveNode.knob("ROI").setValue([0,
                                                                0,
                                                                sel.width(),
                                                                sel.height()])
                                curveNode.setInput(0, shuffleNode)
                                nuke.execute(curveNode,
                                             sel.knob("first").value(),
                                             sel.knob("last").value())

                            if self.autoCrop.getValue() == 1.0 or \
                                    self.autoCropPerChannel.getValue() == 1.0:
                                cropNode = nuke.nodes.Crop(name=channelLayer,
                                                           inputs=[shuffleNode])
                                ...
                                cropNode["name"].setValue(channelLayer)
                            ...
                            # create folders for all layer
                            # and create write node for every shuffle
                            if self.outputPath.getValue()!="":
                                ...
                                write.knob("file").setValue(renderTo+
                                                            channelLayer+
                                                            "/"+
                                                            channelLayer+
                                                            "_%04d.exr")
                                temp = "Zip (16 scanlines)"
                                write.knob("compression").setValue(temp)
