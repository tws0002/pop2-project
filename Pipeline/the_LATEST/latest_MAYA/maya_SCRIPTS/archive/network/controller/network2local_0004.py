...
def test_case(forceRelative, ignoreLocked, newestFilesOnly, 
    ...
    AUTHOR NOTE::
    ::TO DO::
    - make test cases for every type of string replace here
    - Also, make tests to ensure that the current OS matches the OS of the string being queried
      if it doesn't, the string's folders must be replaced as eligently as possible
    """

    """
    AUTHOR NOTE: Check if the file exists, here
    """

    fileIO = checkout.FileCheckoutMaya(filePath=sceneFilePath,
                                       fileName=sceneFileName)

    if fileIO.get_path_os == 'linux2':
        pass
    elif fileIO.get_path_os == 'win32':
        pass
    ...
    # cmds.file(sceneFile, o=True)

    for node in pm.ls():
        # get all of the file formats and then get their string attrs
        if isinstance(node, fileIO.fileStringInputTypes):
            queryAttr = fileIO.fileStringInputInfo[type(node)]
            file_h = pm.getAttr( node + "." + queryAttr)
            print
            print file_h
            yield file_h

    # now that all files are collected, check for nested Maya files and get their contents
    for reference in cmds.file(q=True, r=True):
            if reference.endswith(consts.FILETYPES_MAYA):
                mayaSceneList.append(reference)
            else:
                raise RuntimeError("Not sure what this is")
