...
class PickNodeGeneric(object):
    ...
    def set_attr(self, inputH, value):
        ...
        # if the input isn't a string, assume it's a valid metadata class and handle it
        try:
...
class MotorMaya(object):
    ...
    def has_metadata(self, nodeType=None, name=None):
        ...
        pickNode = PickNodeGeneric(nodeType, name)
        try:
            if not isinstance(pm.PyNode(pickNode.nodeName), nodeType):
                raise pm.MayaNodeError
        except pm.MayaNodeError:
            return False
    ...
    def disconnect(self, allowedNodeAttrs=True, *args, **kwargs):
        ...
        """
        # src = self.pickNode.nodeName + ".message"
