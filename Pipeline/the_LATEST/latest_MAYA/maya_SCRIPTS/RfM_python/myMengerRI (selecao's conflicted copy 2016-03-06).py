...
def init_attributes(selection=None, depth=3, freq=0.5, offset=0,
    ...
    try:
        ...
        pm.addAttr(selection, longName="seed",
                   attributeType="byte")  # :AUTHORNOTE: :WARNING: is an 8-bit int enough?
        pm.addAttr(selection, longName="depth", attributeType="byte")
        pm.addAttr(selection, longName="offset", attributeType="float")
        pm.addAttr(selection, longName="frequency", shortName="freq",
                   attributeType="float")
        pm.addAttr(selection, longName="amplitude",
                   shortName="amp", attributeType="float")
        pm.addAttr(selection, longName="Maximum_Amplitude",
                   shortName="maxAmp", attributeType="float")
    ...
    mel.eval('rmanAddAttr ' + selection.nodeName() +
             ' $attr "myMengerRI(some/path/here/mengerOutput.rib)";')
    return True
# end init_attributes
...
def create_menger(object, rootDir):
    ...
    Args:
        rootDir (str): The directory path, including the intended filename + extension
        ...
        offset (float): A phase operation that will cause the sine to
                        "ripple"
        seed (int): A psuedo-random number generator. Set to a single number
...
def simulate_ribs(object, ribPath, start, end):
    ...
    Args:
        object (pm.nodetypes.Mesh): The shape node to create rib files from
