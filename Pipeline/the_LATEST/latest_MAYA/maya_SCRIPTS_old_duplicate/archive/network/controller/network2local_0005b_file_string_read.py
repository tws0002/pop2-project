...
def test_case(forceRelative, ignoreLocked, newestFilesOnly, 
    ...
    # get all objects with strings, check if they're absolute
    # if absolute and option is set, make them relative using the project path
    # if absolute and not set, flag it for the user to see
    # if relative, append/change against the project path. if that file/folder exists, add it also
    for node in pm.ls():
