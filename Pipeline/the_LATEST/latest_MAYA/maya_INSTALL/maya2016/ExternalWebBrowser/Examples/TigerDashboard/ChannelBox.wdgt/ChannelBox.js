...
if (window.widget)
{
    widget.onshow = doUpdate;
    window.onfocus = doUpdate;
}

function checkConnection() {
	MEL = document.embeds[0];
	if(MEL.port != "commandportDefault") {
		MEL.port = "commandportDefault";
	}
}
...
function doUpdate()
{
	...
	var selected = MEL.execute("ls -sl");
	if(selected.substring(0, 11) == "// WARNING:") {
		document.getElementById("selection").value = "No Selection";
		document.getElementById("txValue").value = "";
		document.getElementById("tyValue").value = "";
		document.getElementById("tzValue").value = "";
		document.getElementById("rxValue").value = "";
		document.getElementById("ryValue").value = "";
		document.getElementById("rzValue").value = "";
		document.getElementById("sxValue").value = "";
		document.getElementById("syValue").value = "";
		document.getElementById("szValue").value = "";
	} else {
		document.getElementById("selection").value = selected;
		document.getElementById("txValue").value = MEL.execute("getAttr .tx");
		document.getElementById("tyValue").value = MEL.execute("getAttr .ty");
		document.getElementById("tzValue").value = MEL.execute("getAttr .tz");
		document.getElementById("rxValue").value = MEL.execute("getAttr .rx");
		document.getElementById("ryValue").value = MEL.execute("getAttr .ry");
		document.getElementById("rzValue").value = MEL.execute("getAttr .rz");
		document.getElementById("sxValue").value = MEL.execute("getAttr .sx");
		document.getElementById("syValue").value = MEL.execute("getAttr .sy");
		document.getElementById("szValue").value = MEL.execute("getAttr .sz");
	}
}

function doSetAttr(attrName)
{
	checkConnection();
	
	var fieldId = attrName + "Value";
	var val = document.getElementById(fieldId).value;
	var command = "setAttr ." + attrName + " " + val;
	MEL.execute(command);
}

function doSelect(attrName)
{
	checkConnection();
	var command = "select " + document.getElementById("selection").value;
	MEL.execute(command);
