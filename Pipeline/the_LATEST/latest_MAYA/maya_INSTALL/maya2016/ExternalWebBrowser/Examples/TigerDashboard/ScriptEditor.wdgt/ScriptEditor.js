...
function doExecute()
{
	...
	var res = MEL.execute(command);
	
	if(res.substring(0, 11) == "// WARNING:") {
...
function doUsage()
{
	...
	var cmd = (selEnd > selStart) ? (field.value).substring(selStart, selEnd) : field.value;
	
	var result = MEL.execute("help " + cmd);
	document.getElementById("ResultField").value = result;
}

function doHelp()
{
	widget.openURL("mel://showHelp -docs index.html");
}
...
function showPrefs()
{
	...
	document.getElementById("portName").value = MEL.port;
