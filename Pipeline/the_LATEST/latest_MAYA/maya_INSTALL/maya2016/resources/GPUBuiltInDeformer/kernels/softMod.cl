    ...
    )
{
	...
	if( normDistance < 1.0 )
	{
		int index = normDistance*20.0;
		float fraction = (normDistance*20.0)-index;
    ...
    )
{
    ...
    if ( falloffRadius <= 0.1 )
	{
	    vstore3( initialPosition , positionId , finalPos );
		return;
	}

	float normDistance = length( initialPosition-falloffCenter )/falloffRadius;
